...
by cpsidp: gen into_unemp = unemp ==1 & unemp[_n-1]==0
...
collapse (sum) employed_st=employed [aw=wtfinl],by(ind1950 statefip)
bysort statefip: egen employed_s = total(employed_st)
gen frac_emp_st_77 = employed_st/employed_s
save "${buildpter}\temp\bartik_iv.dta",replace
//alt
cd "$buildpter\output"
use year employed_alt female ind1950 wtfinl married statefip if year==1977 & married==1 & female==0 & employed_alt==1 using cps_atus_merged_76_21, clear
collapse (sum) employed_st_alt=employed_alt [aw=wtfinl],by(ind1950 statefip)
bysort statefip: egen employed_s_alt = total(employed_st_alt)
gen frac_emp_st_77_alt = employed_st_alt/employed_s_alt
save "${buildpter}\temp\bartik_iv_alt.dta",replace

*merge CPS w/ constructed initial industry shares, construct Bartik
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear
merge m:1 ind1950 statefip using "${buildpter}\temp\bartik_iv.dta"
merge m:1 ind1950 statefip using "${buildpter}\temp\bartik_iv_alt.dta", gen(_merge2)
drop _merge _merge2
*construct bartik
gen bartik_iv_ind = frac_emp_st_77*pter_st // PTER bartik
sort cpsidp monthly_date
gen bartik_iv_unemp = frac_emp_st_77*unemp_st // unemployment bartik
//alt
gen bartik_iv_ind_alt = frac_emp_st_77_alt*pter_st_alt // PTER bartik - alt
sort cpsidp monthly_date
gen bartik_iv_unemp_alt = frac_emp_st_77_alt*unemp_st_alt // unemployment bartik - alt
save"${buildpter}\output\cps_atus_merged_76_21.dta", replace

*leave-out prediction of male pter and unemployment transition rate, basic and alternative version

global var = "pter unemp unemployed"
foreach v of global var{
	
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear
keep if female == 0 & employed == 1
drop if statefip == . // for some obs we don't have state info

cd "${buildpter}/temp/leaveout/`v'"
	
	levelsof statefip, local(state)
	foreach s of local state{
		preserve
		drop if statefip ==`s'
		collapse (mean) into_`v'_exclself = into_`v' [aw=wtfinl], by(year ind1950)
		gen statefip = `"`s'"'
		save leaveout_`s', replace
		restore
		}

*append leave-out means

use leaveout_1, clear
...
drop if statefip == .
save leaveout_all, replace
}

//Alt version
global var = "pter_alt unemp_alt"
foreach v of global var{
	
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear
keep if female == 0 & employed_alt == 1
drop if statefip == . // for some obs we don't have state info
...
cd "${buildpter}/temp/leaveout/`v'"
	...
	foreach s of local state{
		...
		collapse (mean) into_`v'_exclself = into_`v' [aw=wtfinl], by(year ind1950)
		gen statefip = `"`s'"'
		save leaveoutalt_`s', replace
		restore
		}

*append leave-out means

use leaveoutalt_1, clear
capture ssc install fs
...
destring statefip, replace
drop if statefip == .
save leaveoutalt_all, replace
}


*Repeat at year_month level

local var = "pter unemp unemployed"
...
cd "${buildpter}/temp/leaveout/`v'"
	...
	foreach s of local state{		
		preserve
		drop if statefip ==`s'		
		collapse (mean) into_`v'_exclself_md = into_`v' [aw=wtfinl], by(monthly_date ind1950)		
		gen statefip = `"`s'"'	
		save leaveoutmd_`s', replace		
		restore
		}

*append leave-out means
use leaveoutmd_1, clear
clear
fs leaveoutmd_*.dta
append using `r(files)', force
destring statefip, replace
drop if statefip == .
save leaveoutmd_all, replace
...
cd "${buildpter}/temp/leaveout/`v'"
	...
	foreach s of local state{		
		...
		collapse (mean) into_`v'_exclself_md = into_`v' [aw=wtfinl], by(monthly_date ind1950)		
		gen statefip = `"`s'"'	
		save leaveoutmdalt_`s', replace		
		restore
		}

*append leave-out means
use leaveoutmdalt_1, clear
clear
fs leaveoutmdalt_*.dta
append using `r(files)', force
destring statefip, replace
drop if statefip == .
save leaveoutmdalt_all, replace

}


*merge leave-out prediction into full data
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear

global buildpter "C:/Users/ilariamalisan/Dropbox/Hours constraints and home prod/build" // slashes need to be the other way for the loop to work with cd

global var1 = "pter unemp unemployed"
foreach v in $var1{
	merge m:1 statefip year ind1950 using "${buildpter}/temp/leaveout/`v'/leaveout_all.dta", gen(merge_lo`v'all)
	drop if merge_lo`v'all == 2 // no occurrences in original data

	
	merge m:1 statefip monthly_date ind1950 using "${buildpter}/temp/leaveout/`v'/leaveoutmd_all.dta", gen(merge_lo`v'mdall)
	drop if merge_lo`v'mdall == 2 // no occurrences in original data

}

global var2 = "pter_alt unemp_alt"	
foreach v in $var2{
	merge m:1 statefip year ind1950 using "${buildpter}/temp/leaveout/`v'/leaveoutalt_all.dta", gen(merge_lo`v'altall)
	drop if merge_lo`v'altall == 2 // no occurrences in original data

	
	merge m:1 statefip monthly_date ind1950 using "${buildpter}/temp/leaveout/`v'/leaveoutmdalt_all.dta", gen(merge_lo`v'mdaltall)
	drop if merge_lo`v'mdaltall == 2 // no occurrences in original data

}
	

* 33 instances of master only observations: in most cases they have missing frac_emp_st_77, hence missing bartik. Seem to be specific year-state combinations.

label var into_pter_exclself_md "Leave out mean - monthly_date"
label var into_pter_exclself "Leave out mean - yearly date"
label var into_pter_alt_exclself_md "Leave out mean, zero if not in LF - monthly_date"
label var into_pter_alt_exclself "Leave out mean, zero if not in LF - yearly date"
label var into_unemp_exclself_md "Leave out mean using into_unemp - monthly_date"
label var into_unemp_exclself "Leave out mean using into_unemp - yearly date"
label var into_unemp_alt_exclself_md "Leave out mean using into_unemp_alt, zero if not in LF - monthly_date"
label var into_unemp_alt_exclself "Leave out mean using into_unemp_alt, zero if not in LF - yearly date"
label var into_unemployed_exclself_md "Leave out mean using into_unemployed - monthly_date"
label var into_unemployed_exclself "Leave out mean using into_unemployed - yearly date"
