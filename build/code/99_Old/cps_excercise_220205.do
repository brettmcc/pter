...
import delimited cps_asec_76_87.csv
...
**
save cps_asec_76_87.dta, replace
...
egen everasec = max(asecflag), by(cpsid)
order everasec, after(asecflag)


order whyptlwk, after(inctot)
...
gen married = 0 // build changes in marital status? for the time being dummy
...
gen employed = 0
replace employed = 1 if empstat == 10 | empstat == 12  // at work or has jub but not at work last week
...
bys cpsidp year: egen hhincome2 = min(hhincome)
...
bys cpsidp year: egen inctot2 = min(inctot)

foreach j of numlist 1/4{

bys cpsidp: replace inctot2 = inctot2[_n+`j'] if missing(inctot2)
bys cpsidp: replace inctot2 = inctot2[_n-`j'] if missing(inctot2)
}

order inctot2, after(inctot)
*/
