...
save "${buildpter}\temp\cps_atus_merged_76_21.dta", replace
...
save "${build}\temp\bartik_iv.dta",replace

*merge CPS w/ constructed initial industry shares, construct Bartik
use if employed==1 & married==1 using "${buildpter}\temp\cps_atus_merged_76_21.dta",clear
merge m:1 ind1950 statefip using "${buildpter}\temp\bartik_iv.dta"
*construct bartik
gen bartik_iv_ind = frac_emp_st_77*pter_st
sort cpsidp monthly_date
save "${buildpter}\output\cps_atus_merged_76_21.dta",replace


*leave-out prediction of male pter transition rate
use cps_atus_merged_76_21, clear

*keep if year == 1977 & statefip == 1 & ind1950 == 367

collapse (sum) into_pter_sti = into_pter [aw=wtfinl], by(statefip year ind1950) // total pter Alabama 1980 mines
collapse (mean) into_pter_sti = into_pter [aw=wtfinl], by(statefip year ind1950) // total pter Alabama 1980 mines

bysort year ind1950: egen into_pter_ind = total(into_pter_sti) // yearly total across states by industry -- check if there is a difference with sum
bysort year state: egen into_pter_state = total(into_pter_sti) // state total
bysort year: egen into_pter_year = total(into_pter_sti) // yearly total

gen st_wgt = into_pter_state / into_pter_year // state share of total pter, accounting for state-year size

levelsof year
return list
global year = r(levels)
disp "$year"

levelsof statefip
return list
global state = r(levels)
disp "$state" // reaches 56 but total unique levels is 51, ok

levelsof ind1950
return list
global industry = r(levels)
disp "$industry"



*(total PTER in industry i at time t across all states - PTER in industry i at time t in state s) / total PTER in industry i at time t across all states
ssc install asgen
gen ip_nonself = .
sort year ind1950 statefip

/*
levelsof year, local(year)
foreach t of local year{
	levelsof statefip, local(state)
	foreach s of local state{
		levelsof ind1950, local(industry)
...
foreach s in $state{
	foreach t in $year{
		foreach i in $industry{
			...
			asgen temp = into_pter if year == `t' & statefip != `s' & ind1950 == `i', w(wtfinl) 
			*gen prova_`s' = 0
			gen prova_`s' = temp 
			replace if statefip == `s' &  year == `t' & ind1950 == `i'
	...
	foreach s of local state{
		...
		foreach t of local year{
			...
			foreach i of local industry{
				...
				sum into_pter [aw=wtfinl] if year == `t' & statefip != `s' & ind1950 == `i', meanonly
				scalar wtmean = r(mean)
				replace temp_`s' =  wtmean if statefip == `s' &  year == `t' & ind1950 == `i'
				scalar drop wtmean
...
replace ip_nonself = 0 if ip_nonself < 0 | ip_nonself == .

gen into_pter_pred = ip_nonself * st_wgt //weighting by state-year size

save leaveout.dta, replace

*merge leave-out prediction into full data
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear

merge m:1 statefip year using "${build}\temp\leaveout.dta", nogen
