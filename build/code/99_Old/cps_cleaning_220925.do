...
*global buildpter "C:\Users\Brett\Dropbox\Hours constraints and home prod\build"
cd "$buildpter\input\CPS-ATUS"

*global builtpter "C:\Users\ilariamalisan\Downloads" // workstation
...
//Unemployed
gen unemp = 0
replace unemp = 1 if empstat == 21 | empstat == 22 // unemployed: experienced worker or new worker
...
gen into_unemployed = employed==0 & l1.employed==1
...
use "${buildpter}\output\cps_atus_merged_76_21.dta", clear
merge m:1 statefip year ind1950 using leaveout_all, gen(merge_leaveout)
drop if merge_leaveout == 2 // no occurrences in original data

cd "${buildpter}\temp"
save cps_atus_merged_76_21_leaveout, replace
...
cd "${buildpter}\temp\leaveout"
	...
	foreach s of local state{		
		preserve
		drop if statefip ==`s'		
		collapse (mean) into_pter_exclself_md = into_pter [aw=wtfinl], by(monthly_date ind1950)		
		gen statefip = `"`s'"'	
...
*merge leave-out prediction into full data
use "${buildpter}\temp\cps_atus_merged_76_21.dta", clear
merge m:1 statefip monthly_date ind1950 using "${buildpter}\temp\leaveout\leaveoutmd_all.dta", gen(merge_leaveoutmd)
drop if merge_leaveoutmd == 2 // no occurrences in original data
