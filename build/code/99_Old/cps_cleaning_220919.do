...
use year employed female married statefip into_pter wtfinl if female == 0 & employed == 1 using "${buildpter}\output\cps_atus_merged_76_21.dta", clear
collapse (mean) into_pter_avg = into_pter [aw=wtfinl], by(statefip year)
rangestat into_pter_leaveout = into_pter_avg (avg), interval(year 0 0) excludeself // should I weight this on pop or is it avg of avg?
drop into_pter_avg_mean

save "${build}\temp\leaveout.dta", replace
