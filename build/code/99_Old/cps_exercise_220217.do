...
import delimited cps_asec_76_21_v2.csv // asec pre 75 is non linkable between years but we don't exploit that dimension
...
foreach j of numlist 1/4{
bys cpsidp: replace famincome_sp = famincome_sp[_n+`j'] if missing(famincome_sp)
bys cpsidp: replace famincome_sp = famincome_sp[_n-`j'] if missing(famincome_sp) // check with brett
}
...
bys cpsidp: replace famincome_married = famincome_married[_n-`j'] if missing(famincome_married) // check with brett
}
...
/*
    ...
    Result                           # of obs.
    -----------------------------------------
    not matched                    42,074,697
        from master                42,073,957  (_merge_atus==1)
        from using                        740  (_merge_atus==2)

    matched                         1,154,834  (_merge_atus==3)
...
------------------------+-----------------------------------
        master only (1) | 18,126,873       94.01       94.01
         using only (2) |        740        0.00       94.01
            matched (3) |  1,154,834        5.99      100.00
------------------------+-----------------------------------
                  Total | 19,282,447      100.00
*/
...
/*
    Result                           # of obs.
    -----------------------------------------
    not matched                    42,589,039
        from master                42,523,958  (_merge_atus_sp==1)
        from using                     65,081  (_merge_atus_sp==2)

    matched                           705,573  (_merge_atus_sp==3)
    -----------------------------------------
...
// Either one spouse is PTER
* -- need info on whether spouse is pter -- merge again.

// create temp file with just cpsid (which I will merge with main files' cpsidp to attach spouse info) and pter -- this would be pter spouse, and homeworkhrs -- homeworkhours for spouse

keep pter cpsidp uhrswork1 uhrsworkt ahrsworkt homeworkhrs 

rename cpsidp cpsidp_sp
rename pter pter_sp
rename uhrswork1 uhrswork1_sp
rename uhrsworkt uhrsworkt_sp
rename ahrsworkt ahrsworkt_sp
rename homeworkhrs homeworkhrs_sp

// Merging spouse pter and hours worked back into cps (=same file)
merge m:m cpsidp_sp using cps_atus_merged_76_21, gen(_merge_cps_sp)
...
*** Adding number of people in family 
